...
export function createSettingsGUI(preset, container, onChange) {
  ...
  // ── Emboss ─────────────────────────────────────────────
  const embossTitle = document.createElement('h4');
  embossTitle.textContent = 'Emboss';
  container.appendChild(embossTitle);
  addCheckbox('Enabled', preset.emboss, 'enabled');
  addSlider('Strength', preset.emboss, 'strength', 0, 1, 0.01);
  addSlider('Angle (°)', preset.emboss, 'angle', 0, 360, 1);
  addSlider('Detail Scale', preset.emboss, 'detailScale', 0.5, 5, 0.1);
  addSlider('Edge Threshold', preset.emboss, 'edgeThreshold', 0.0, 0.5, 0.01);  // <-- new slider
  ...
  resetBtn.addEventListener('click', () => {
    const defaultPreset = {
      ...
      emboss: {
        ...
        angle: 45,
        detailScale: 1.0,
        edgeThreshold: 0.05      // <-- default edge threshold
      }
    };
