...
import { createEmbossEffect } from './embossEffect.js';

let composer, bloom, dof, brightnessContrast, hueSaturation, vignette, noise, smaa, clampEffect;
let chromaticAberration, velocityBlurEffect, embossEffect;
...
const EMBOSS_DEFAULT_BLEND = BlendFunction.OVERLAY;
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  for(float i = -stepCount; i <= stepCount; i += 1.0) {
    ...
    vec2 offset = direction * intensity * i * 0.003;
...
export async function initPostProcessing(renderer, scene, camera) {
  ...
  chromaticAberration.blendMode.blendFunction = BlendFunction.SKIP;
  ...
  // ── Emboss effect ─────────────────────────────────────────
  embossEffect = createEmbossEffect();
  ...
  const effectPass = new EffectPass(
    ...
    chromaticAberration,
    velocityBlurEffect,
    embossEffect,          // before SMAA so anti‑aliasing applies
    smaa,
  );
...
function updateComposer(preset) {
  ...
  // ── Emboss ────────────────────────────────────────────────
  if (preset.emboss) {
    setEffectEnabled(embossEffect, preset.emboss.enabled, EMBOSS_DEFAULT_BLEND);
    embossEffect.uniforms.get('strength').value = preset.emboss.strength;
    const angleRad = THREE.MathUtils.degToRad(preset.emboss.angle);
    embossEffect.uniforms.get('lightDirection').value.set(
      Math.cos(angleRad),
      Math.sin(angleRad)
    );
    embossEffect.uniforms.get('detailScale').value = preset.emboss.detailScale;
  }
}
