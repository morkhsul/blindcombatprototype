...
let chromaticAberration, velocityBlur;
const remoteMeshes = {};
let dummyMesh;

// ── HDRI toggle state ──────────────────────────
let environmentMap = null;          // stored PMREM environment map
let hdriAmbientLight = null;       // fallback ambient light when HDRI is off
...
export async function initRenderer() {
  ...
  environmentMap = envMap;          // store for toggling
  pmremGenerator.dispose();
  exrTexture.dispose();

  // Fallback ambient light (not added to scene yet; used when HDRI is toggled off)
  ...
  normalGround = new THREE.Mesh(groundGeo, groundMat);
  ...
  window.toggleHDRI = toggleHDRI;   // for console use: toggleHDRI()
